...
serverWs.addListener("connection", (socket, req) => {
  ...
  const disconnectUser = function () {
    for (let entry of Object.entries(subscribers)) {
      if (entry[1] === socket) {
        ...
        manageEvent(event, socket);
        break;
      }
    }
  };
  socket.onclose = disconnectUser;
  socket.onerror = disconnectUser;
});
...
function manageEvent(event, ws) {
  if (event.type.startsWith("webrtc-")) {
    forwardEvent(event, event.receiver);
  } else
    switch (event.type) {
      case "join":
      ...
      case "move":
        ...
        const meet = createMeeting(event.player);
        if (meet) {
          console.log("Player meet : ", meet);
          broadcast(meet, -1);
        }
        broadcast(event, event.player.name);
        break;
      case "leave":
        ...
        delete playersInMeeting[event.player.name];
        broadcast(event, event.player.name);
        break;
      default:
...
function forwardEvent(event, name) {
  if (name in subscribers) {
    subscribers[name].send(JSON.stringify(event));
  }
}

const playersInMeeting = {};
function createMeeting(player) {
  if (playersInMeeting[player.name]) {
    return checkAway(player);
  }

  for (let playerName in playersData) {
    if (playerName !== player.name && !playersInMeeting[playerName]) {
      const meet = isNear(player, playersData[playerName]);
      if (meet) {
        return meet;
      }
    }
...
function isNear(player1, player2) {
  ...
  if (distance < 100) {
    const meet = {
      ...
      radius: 50,
      creator: player1.name,
      elements: [player1.name, player2.name],
    };
    playersInMeeting[player1.name] = meet;
    playersInMeeting[player2.name] = meet;
